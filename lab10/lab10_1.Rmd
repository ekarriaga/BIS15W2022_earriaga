...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) %>% 
  ggplot(aes(x=reorder(order,mean_mass), y=mean_mass))+ geom_col()+coord_flip()+scale_y_log10()#aloows to visualize easier with wide range of counts 
```
...
```{r}
life_history %>% 
  ggplot(aes(x=order, y=mass))+geom_boxplot()+scale_y_log10()+coord_flip()#remember scale y helps account for range of values
```
...
We can improve the plot further by adjusting the size and face of the text. We do this using `theme()`. FONT SIZE
```{r}
elephants %>% 
  ggplot(aes(x=age, y=height)) + geom_point() + geom_smooth(method=lm, se=F) +
  labs(title = "Elephant Age vs. Height",
       x = "Age",
       y = "Height") +
  theme(plot.title = element_text(size = 20, face = "italic","underline"),
...
elephants %>% 
  ...
  labs(title = "Elephant Age vs. Height",
       x = "Age",
       y = "Height") +
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))# rel is the relative size, scals titale and axis relative to each other
```
...
life_history %>% 
  ...
  geom_point(na.rm=T) #size does not identify the outlier but its easy to understand 
```
...
homerange%>%
  ...
  labs(title = "Ectotherms vs. Endotherms", x= "Thermoregulation", y= "Count")+
  theme(plot.title = element_text(size = 20, face = "italic","bold"),
        axis.text = element_text(size = 12),
        axis.title = element_text(size = 12))
```
ggplot(aes(x=age, y=height)) + geom_point() + geom_smooth(method=lm, se=F) +
...
homerange%>%
  ...
  geom_jitter()+
  labs(title = "Homerange vs. Mass", x= "Mass (log10)", y= "Homerange (log 10 hra)")
  
  # alpha allows transperancy of points, size canges the size of point, colo give color
```
