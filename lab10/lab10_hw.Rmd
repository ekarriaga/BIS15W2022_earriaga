...
deserts%>%
  filter(weight!= "NA")%>%
  ggplot(aes(x=species, y=weight))+
  geom_point(na.rm = T)+ scale_y_log10()+
  coord_flip()
```
...
9. Which two species have, on average, the highest weight? Once you have identified them, make a new column that is a ratio of `weight` to `hindfoot_length`. Make a plot that shows the range of this new ratio and fill by sex. to make a new column we have o use  mutate
```{r}
deserts %>% 
