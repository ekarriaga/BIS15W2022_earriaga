...
#csv is not preparitory, many programs can read csv files. 
...
files
```
# creates a new object of all the files 
...
names #temporary object we are using 
```
...
```{r}
filter(spider_list$Butte) #use base R, don't have to use filter, no brackets either
```
...
```
```{r}
table_B # were missing colums from data but we can join 
```
...
inner_exampleDF #we are missig 3 and 4, if you join and one is missing it will drop it out, this can be helpful ot no helpful.
```
...
left_exampleDF #we are joining next to the left
```
...
full_exampleDF # puts everything together, mostly use full join and accept the NAs, work afterward. 
```
...
## Practice
1. Which type of join is most appropriate in this case?  #any of the joins would work for this 
```{r}
...
```{r}
day <- today()
day #year month day, in that order 
```
...
# this is not lubridate, just basic R
```
`
