...
We won't use the name `species` because that will be the `ggplot` grouping variable. Copy and paste the code below and run the app to see our new input. We wont use sex or year as categorical 
```{r, eval=F, echo=T}
ui <- fluidPage(
...
ui <- fluidPage(titlePanel("Homerange Locomotion"),
                radioButtons("x", "Select Fill Variable", choices=c("trophic.guild", "thermoregulation"), selected="trophic.guild"), #selected tells us where the button will frist start at 
...
server <- function(input, output, session) {
  ...
  })#this is where we put our normal plot data #alpha is for transparency 
