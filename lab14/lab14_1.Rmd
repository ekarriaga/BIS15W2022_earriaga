...
penguins %>% 
  ...
  geom_violin(alpha = .5, na.rm = TRUE) + #we can lay the plots on top of each other
  coord_flip()
```
...
Cool! Now we see that there are actually some of the smallest penguins on Biscoe island. This where we get our nonnormal shape can be derived my species, but it could also be age(its not within the data)
...
the box plot may be missing how many of sexes are identified within the taxon, can cause a shift in distribution. 
...
```
we see in the plot that drug d lowers your heart rate, 

## Practice
  1. Which drug lowers heart rate? raises it?

  2. Make a heat map for body mass (still) by species & sex for penguins
