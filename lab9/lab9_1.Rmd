...
In the case below, we are exploring whether or not there is a relationship between animal mass and home range. We are using the **log transformed values** because there is a large difference in mass and home range among the different species in the data. #using logmass makes it easier to visaulize
```{r}
names(homerange)
```
...
homerange %>% 
  ...
  select(common.name, log10.mass)%>%# dont need to add this, only want to sepcifiy 
